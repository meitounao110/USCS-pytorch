...
class _SimpleSegmentationModel(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.classifier(features, low_level_features)
...
class IntermediateLayerGetter(nn.ModuleDict):
    """
    Module wrapper that returns intermediate layers from a modeling

    It has a strong assumption that the modules have been registered
    into the modeling in the same order as they are used.
    This means that one should **not** reuse the same nn.Module
    twice in the forward if you want this to work.

    Additionally, it is only able to query submodules that are directly
    assigned to the modeling. So if `modeling` is passed, `modeling.feature1` can
    be returned, but not `modeling.feature1.layer2`.

    Arguments:
        model (nn.Module): modeling on which we will extract the features
    ...
    def __init__(self, model, return_layers, hrnet_flag=False):
        if not set(return_layers).issubset([name for name, _ in model.named_children()]):
            raise ValueError("return_layers are not present in modeling")
